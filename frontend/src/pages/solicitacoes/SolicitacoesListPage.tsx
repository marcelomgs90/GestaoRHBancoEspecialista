  ...
  return (
    <div className="space-y-6 sm:space-y-8 animate-in slide-in-up">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-slate-200 pb-5 sm:pb-6">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-slate-950">Solicitações de RH</h2>
        ...
        {podeEditarMembros && (
          <button
            ...
            className="flex w-full items-center justify-center px-4 py-2 bg-slate-900 text-white rounded font-bold text-[10px] uppercase tracking-wider hover:bg-slate-800 transition-all shadow-sm whitespace-nowrap cursor-pointer sm:w-auto"
          >
      ...
      <div className="grid grid-cols-1 gap-3 sm:flex sm:items-center sm:flex-wrap">
        <Filter size={14} className="hidden text-slate-400 shrink-0 sm:block" />
        <select
          value={filtroProjeto}
          onChange={(e) => setFiltroProjeto(e.target.value === '' ? '' : Number(e.target.value))}
          className="w-full px-3 py-2 sm:py-1.5 bg-white border border-slate-200 rounded text-xs font-medium outline-none focus:border-slate-400 transition-all text-slate-700 sm:max-w-xs"
        >
            ...
            <div className="space-y-3">
              {filtradas.map((sol, idx) => (
                <motion.div
                  ...
                  className="group bg-white border border-slate-200 p-4 sm:p-6 rounded-lg hover:border-slate-400 hover:shadow-md transition-all cursor-pointer"
                >
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    <div className="flex-1 space-y-1">
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        <h3 className="text-sm sm:text-base font-bold text-slate-900 group-hover:text-blue-600 transition-colors break-words">
                          {sol.identificador}
                        </h3>
                      </div>
                    ...
                    <div className="grid grid-cols-1 gap-4 border-t border-slate-100 pt-4 sm:grid-cols-3 sm:gap-5 lg:flex lg:items-center lg:border-t-0 lg:pt-0 lg:gap-6">
                      ...
                      {podeEditarMembros && sol.status === StatusSolicitacao.EM_EDICAO && (
                        <button
                          ...
                          className="flex items-center justify-center gap-1.5 px-3 py-2 sm:py-1.5 bg-blue-50 hover:bg-blue-600 hover:text-white border border-blue-200 hover:border-blue-600 rounded text-[10px] font-bold uppercase tracking-wider transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed sm:col-span-3 lg:col-span-1"
                        >
